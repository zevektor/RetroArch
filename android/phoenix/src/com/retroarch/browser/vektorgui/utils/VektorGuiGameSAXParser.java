...
public class VektorGuiGameSAXParser {

	public static ArrayList<com.retroarch.browser.vektorgui.utils.Serialization.gameClass> parse(String xml) {
		SAXParserFactory spf = SAXParserFactory.newInstance();
		ArrayList<gameClass> output = new ArrayList<com.retroarch.browser.vektorgui.utils.Serialization.gameClass>();
	...
	static class XMLHandler extends DefaultHandler {
		...
		private boolean in_releasedate = false;
		private ArrayList<gameClass> output = new ArrayList<gameClass>();
		private gameClass gc = new gameClass(null, null, null, null);
		private int games = 0;
		...
		public void characters(char ch[], int start, int length) {
			...
			if (in_game) {
				...
				} else if(in_releasedate){
					String date = textBetween;
					if(null!=date && date.split("/").length>=3){
						gc.setYear(date.split("/")[2]);
					}
				} else if(in_overview){
					gc.setDescription(textBetween);
				} 
			} else if(in_baseimgurl){
				baseImgURL = textBetween;
			} else {
				if (null != gc.getTitle() && null != gc.getURL()) {
					output.add(new gameClass(gc.getTitle(), gc.getURL(), gc.getDescription(), gc.getYear()));
					games++;
					gc.clear();
				}
			}
