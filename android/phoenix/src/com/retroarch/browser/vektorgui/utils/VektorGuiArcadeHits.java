...
public class VektorGuiArcadeHits {
	...
	private void getCoverLink() {
		try {
			Document doc = Jsoup
					.connect(
							"http://arcadehits.net/index.php?p=roms&jeu="
									+ item.getGameName()).timeout(15000).get();
			Elements els = doc.getElementsByTag("h4");
			String gameName = els.get(0).text();
			if ("Derniers jeux commentés".equalsIgnoreCase(gameName))
				return;
			els = doc.select("a[href~=yearz]");
			if (els.size() > 0)
				item.setGameYear(els.get(0).text());
			item.setGameName(gameName);
			item.setGameDescription(activity.getResources().getString(
					R.string.vektor_gui_game_no_description));
			try {
				File props = new File(resStor,
						VektorGuiPlatformHelper.cleanName(item.getROMPath()
								.getName()) + ".prop");
				item.toProperties().store(new FileWriter(props), "");
			} catch (IOException e) {
			} 
			els = doc.select("img.minithumb");
			String coverURL = null;
			if (els.size() > 0)
				coverURL = baseURL + els.get(0).attr("src");
			if (coverURL != null) {
				coverURL = coverURL.replace("http://", "");
				coverURL = ("http://" + Uri.encode(coverURL)).replace("%2F",
						"/");
				File cover = new File(resStor,
						VektorGuiPlatformHelper.cleanName(item.getROMPath()
								.getName()) + "-CV.jpg");
				...
				activity.addGameToReceiver(mManager.enqueue(req), item);
			}
